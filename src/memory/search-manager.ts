import { resolveMemoryManagerKeyForScope } from "../multitenant/integration/memory-hook.js";
...
function resolveScopedOrLegacyQmdCacheKey(agentId: string, resolved: ResolvedQmdConfig): string {
  const legacyKey = buildQmdCacheKey(agentId, resolved);
  const scopedKey = resolveMemoryManagerKeyForScope(getCurrentRuntimeScope() ?? undefined, legacyKey);
...
export async function getMemorySearchManager(params: {
  ...
  if (resolved.backend === "qmd" && resolved.qmd) {
    ...
    const baseCacheKey = resolveScopedOrLegacyQmdCacheKey(params.agentId, resolved.qmd);
